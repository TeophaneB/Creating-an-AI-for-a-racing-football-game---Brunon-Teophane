       ...
       : AIBaseController(kart)
{
    ...
    m_world = dynamic_cast<SoccerWorld*>(World::getWorld());    // used for fetching data and fcts
    m_ai_model = new ai_model();    // my AI class to handle the AI model
}   // ArenaAI
...
//-----------------------------------------------------------------------------
void ArenaAI::random_actions() {    // assign random values to a kart (used for testing)
    //std::cout << "ArenaAI::random_actions" << std::endl;

    float randomSteer = static_cast<float>(rand()) / static_cast<float>(RAND_MAX) * 2.0f - 1.0f;
    float randomAccel = static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
    bool randomBrake = rand() % 2;
    bool randomNitro = rand() % 2;

    m_controls->setAccel(randomAccel);      // [0,1]
    m_controls->setBrake(randomBrake);      // [false,true]
    m_controls->setSteer(randomSteer);      // [-1,1]
    m_controls->setNitro(randomNitro);      // [false,true]
}


//-----------------------------------------------------------------------------
void ArenaAI::isTestSwitch() {  // if the AI is switched, swap the AI model values to opposite team
    if (m_world->isSwitchAi()) {
        bool tempAIBLue = m_ai_model->blueAI;                   // save prev values
        bool tempInputBlue = m_ai_model->inputNoiseBlue;
        bool tempOutputBlue = m_ai_model->outputNoiseBlue;
        bool tempTrainingBlue = m_ai_model->trainingNoiseBlue;

        m_ai_model->blueAI = m_ai_model->redAI; 			    // swap values
        m_ai_model->inputNoiseBlue = m_ai_model->inputNoiseRed;
        m_ai_model->outputNoiseBlue = m_ai_model->outputNoiseRed;
        m_ai_model->trainingNoiseBlue = m_ai_model->trainingNoiseRed;

        m_ai_model->redAI = tempAIBLue;
        m_ai_model->inputNoiseRed = tempInputBlue;
        m_ai_model->outputNoiseRed = tempOutputBlue;
        m_ai_model->trainingNoiseRed = tempTrainingBlue;

        std::cout << "Switched AI: blueAI set to " << m_ai_model->blueAI << std::endl;
        std::cout << "Switched: inputNoiseRed set to " << m_ai_model->inputNoiseRed << std::endl;
        std::cout << "Switched: outputNoiseRed set to " << m_ai_model->outputNoiseRed << std::endl;
        std::cout << "Switched: trainingNoiseRed set to " << m_ai_model->trainingNoiseRed << std::endl;

        std::cout << "Switched AI: redAI set to " << m_ai_model->redAI << std::endl;
        std::cout << "Switched: inputNoiseBlue set to " << m_ai_model->inputNoiseBlue << std::endl;
        std::cout << "Switched: outputNoiseBlue set to " << m_ai_model->outputNoiseBlue << std::endl;
        std::cout << "Switched: trainingNoiseBlue set to " << m_ai_model->trainingNoiseBlue << std::endl;
    }
}
...
{
    isTestSwitch();  // check if the AI is to be switched at every tick
...
std::vector<float> ArenaAI::getData() {     // data fetching of current kart to be used for AI model & gathering data for training
    // Data to add
    const Vec3 ball_pos = m_world->getBallPosition();
    const bool ball_not_moving = m_world->ballNotMoving();
    const Vec3 kart_pos = m_kart->getXYZ();
    const float kart_speed = m_kart->getSpeed();
    const bool kart_accel = m_kart->getController()->getControls()->getAccel();
    const float kart_steer = m_kart->getController()->getControls()->getSteer();
    const float kart_max_speed = m_kart->getCurrentMaxSpeed();
    const bool kart_boost = m_kart->getBoostAI();
    const bool kart_brake = m_kart->getController()->getControls()->getBrake();
    const bool kart_nitro = m_kart->getController()->getControls()->getNitro();
    const float kart_steer_angle = m_kart->getMaxSteerAngle();
    findClosestKart(0, 0); // so closest kart vals are accurate
    bool blue_team_reach_goal = m_world->ballApproachingGoal(KART_TEAM_BLUE);
    bool red_team_reach_goal = m_world->ballApproachingGoal(KART_TEAM_RED);
    Vec3 ball_aim_pos;
    if (m_world->getKartTeam(m_kart->getWorldKartId()) == KART_TEAM_BLUE) {
        ball_aim_pos = m_world->getBallAimPosition(KART_TEAM_BLUE);
    }
    else {
        ball_aim_pos = m_world->getBallAimPosition(KART_TEAM_RED);
    }

    // Transform to same data type -> floats
    std::vector<float> data = {
        static_cast<float>(m_kart->getWorldKartId()),
        kart_pos.getX(), kart_pos.getY(), kart_pos.getZ(),
        static_cast<float>(kart_accel), kart_steer, kart_steer_angle,
        kart_speed, kart_max_speed, static_cast<float>(kart_boost),
        static_cast<float>(kart_brake), static_cast<float>(kart_nitro),
        ball_pos.getX(), ball_pos.getY(), ball_pos.getZ(), static_cast<float>(ball_not_moving),
        m_closest_kart_point.getX(), m_closest_kart_point.getY(), m_closest_kart_point.getZ(),
        static_cast<float>(blue_team_reach_goal), static_cast<float>(red_team_reach_goal), 
        ball_aim_pos.getX(), ball_aim_pos.getY(), ball_aim_pos.getZ()
    };
