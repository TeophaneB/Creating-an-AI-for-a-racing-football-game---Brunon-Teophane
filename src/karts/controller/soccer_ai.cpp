...
void SoccerAI::update(int ticks)
{
    ...
    float currTime = m_world->getTime();

    // Data saving for model training
    // Check if it's been at least 0.5 second since the previous write
    if (currTime - m_kart->getPrevSaveTime() >= 0.5) {
        m_kart->setPrevSaveTime(currTime);
        m_world->setData(getData());
    }
...
//-----------------------------------------------------------------------------
bool SoccerAI::isAI() { // determine if current kart is my AI
    if (m_ai_model->blueAI && (m_world->getKartTeam(m_kart->getWorldKartId()) == KART_TEAM_BLUE)) {
        return true;
    }
	else if (m_ai_model->redAI && (m_world->getKartTeam(m_kart->getWorldKartId()) == KART_TEAM_RED)) {
		return true;
	}
    else { return false; }
}
...
void SoccerAI::findTarget()
{
    if (isAI()) {   // if my AI
        float currTime = m_world->getTime();
        if (currTime - m_kart->getPrevModelTime() >= 0.10) {    // if it's been at least 0.1 second since last model call
            m_kart->setPrevModelTime(currTime);
            findTargetAI(); // use the model
        }
    }
    else {
...
//-----------------------------------------------------------------------------
void SoccerAI::findTargetAI() {     // my AI version of the find target
    //std::cout << "SoccerAI::findTargetAI" << std::endl;

    // -_-_- VERSION 1 -_-_-

    
    //std::vector<float> kart_data = getData();
    //Vec3 target_point = m_ai_model->getTargetAI(kart_data);

    //std::cout << " kart predicting -> " << kart_data.at(0) << std::endl;
    ...
    //int closest_node = m_world->getBallNode();
    //float closest_distance = (m_world->getBallPosition() - target_point).length_2d();
    //Vec3 final_position = m_world->getBallPosition();

    //const unsigned int n = m_world->getNumKarts();
    //for (unsigned int i = 0; i < n; i++) {
    //    const AbstractKart* kart = m_world->getKart(i);
    //    int kart_node = m_world->getSectorForKart(kart);
    //    final_position = kart->getXYZ();
    //    float distance = (final_position - target_point).length_2d();

    //    if (distance < closest_distance) {
    //        closest_distance = distance;
    //        closest_node = kart_node;
    //    }
    //}

    //m_target_node = closest_node;
    ////m_target_point = final_position;

    // -_-_- VERSION 2 -_-_-

    Vec3 target_point = m_ai_model->getTargetAI(getData());  // call the model

    // continue the previous logic with the new target point
